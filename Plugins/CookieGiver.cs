...
namespace Twitch.Plugins
{
    public class CookieGiver
    {
        ...
        public void OnPrivateMessage(TwitchClient client, TwitchMessage message)
        {
        ...
        {
			client.Say(message.Channel, "Database contains {0} cookies", m_list_of_cookies.Count());
		}
                ...
                if (msg.Length > "!newcookie ".Length)
                {
                    ...
                    if (sub.ToLowerInvariant().Contains("cookie"))
                    {
						if(CheckAllowedCookie(sub))
						{
							...
							client.Say(message.Channel, "\"{0}\" have been added to the cookie database", sub);
						}
						else 
						{
							client.Say(message.Channel, "Sorry {0}, but you're not allowed to add this.", message.SenderDisplayName);
						}
                    }
                    else
                    {
                        client.Say(message.Channel, "Sorry {0}, but the new flavor must contain the word \"cookie\"", message.SenderDisplayName);
                    }
                }
                else
                {
                    client.Say(message.Channel, "Please specify what you want to add in the cookie database");
                }
            }
            else
            {
                client.Say(message.Channel, "Sorry {0}, this command is for mods and above only.", message.SenderDisplayName);
            }
        }

        private void DisplayCommands(TwitchClient client, TwitchMessage message)
        {
            client.PrivMsg(message.Channel, "Commands are : !cookie !cookiecount !cookiedelay !randomtopic. Type !help <command> for usage");
        }
